...
export async function seedDatabase() {
  try {
    // Insert packages with whole number prices
    await sql`
      INSERT INTO packages (id, name, price, duration_hours, description, features) VALUES
      (1, 'Mint Pro', 299, 2, 'Our entry-level package for small events or budgets.', 
      ...
      (2, 'Mint Max', 399, 3, 'A longer session with extra flair for most events.',
      ...
      (3, 'Mint Ultra', 599, 4, 'The ultimate, all-inclusive photobooth experience.',
    ...
    // Insert add-ons with whole number prices
    await sql`
      INSERT INTO addons (id, name, price, description, category, is_hourly, available_packages) VALUES
      (1, 'Booth Attendant', 49, 'Professional attendant per hour', 'service', TRUE, '[1,2,3]'),
      (2, 'Guest Scrapbook Service', 119, 'Physical scrapbook creation', 'service', FALSE, '[1,2,3]'),
      (3, 'Live Slideshow Station', 119, 'Real-time photo display', 'equipment', FALSE, '[1,2]'),
      (4, 'Unlimited Prints', 149, 'On-site photo printing', 'service', FALSE, '[1,2,3]'),
      (5, 'Green-Screen / AI Scene Pack', 79, 'Digital background effects', 'digital', FALSE, '[1,2]'),
      (6, 'Idle / Early Setup', 49, 'Setup time per hour', 'service', TRUE, '[1,2,3]'),
      (7, 'Premium Backdrop', 99, 'Upgraded backdrop option', 'equipment', FALSE, '[1,2]'),
      (8, 'Virtual Booth', 49, 'Remote photo booth access', 'digital', FALSE, '[1,2,3]'),
      (9, 'Additional Hour', 99, 'Extend booth rental per hour', 'service', TRUE, '[1,2,3]')
      ON CONFLICT (id) DO NOTHING
    `
  ...
  try {
    await sql`
      INSERT INTO bookings (
        ...
        ${JSON.stringify(bookingData.selected_addons)}, ${Math.round(bookingData.subtotal)},
        ${Math.round(bookingData.total_amount)}, ${bookingData.status}, 
...
// Fallback mock data functions with whole number prices
