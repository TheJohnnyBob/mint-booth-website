...
import { Clock, AlertTriangle, CheckCircle, MapPin, Users } from "lucide-react"
...
export interface Booking {
  ...
  package_id: number
  selected_addons: Record<number, number> // { addonId: quantity }
  subtotal: number // Price in cents
  total_amount: number // Price in cents
...
export interface BookingData {
  ...
  eventTime: string
  eventType: string
  guestCount: string // Use string for input compatibility
  venueAddress: string
...
export default function CustomBookingSystem() {
  ...
  const availableAddons = useMemo(
    () => addons.filter((addon) => selectedPackage && addon.available_packages.includes(selectedPackage.id)),
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : "An unknown error occurred. Please refresh the page.")
      } finally {
        setLoading(false)
      }
    }
              ...
              <div>
                ...
                <div className="text-center mt-8">
                  <Button
                    ...
                    disabled={bookingData.packageId === null}
                    size="lg"
              ...
              <div>
                            ...
                            <div className="text-right">
                              ...
                              {currentQuantity > 0 && (
                                <div className={`text-sm ${hasViolation ? "text-red-600" : "text-gray-600"}`}>
                                  Total: ${formatPrice((addon.price * currentQuantity) / 100)}
                                  {hasViolation && (
                ...
                <div className="glass-panel-mint p-6 rounded-2xl mb-8">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="font-bold">${formatPrice(selectedPackage?.price / 100 || 0)}</span>
                  </div>
                        ...
                        <span className="text-gray-900 font-medium">
                          {quantity > 1 ? (
                            <span className="text-sm">
                              ${formatPrice(addon.price / 100)} × {quantity} = ${formatPrice(totalPrice / 100)}
                            </span>
                          ) : (
                ...
                <div className="mt-8 glass-panel-mint p-8 rounded-2xl">
                  ...
                  <div className="bg-white/10 rounded-xl p-4 mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="font-bold text-gray-900 text-lg">
                        ${formatPrice(selectedPackage?.price / 100 || 0)}
                      </span>
                    </div>
                  ...
                  <div className="border-t border-white/20 pt-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-gray-900">${formatPrice(totalPrice / 100)}</span>
                    </div>
                  </div>
                </div>
